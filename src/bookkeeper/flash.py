...
class Simulation(bookkeeper.Simulation):
    ...
    def checkpoints(self) -> list[Checkpoint]:
        ...
        # print([Checkpoint(pathlib.Path(f)) for f in checkpoint_files])
        # exit(0)
        return [Checkpoint(path) for path in self.checkpoint_paths]

    @property
    def checkpoint_paths(self) -> list[pathlib.Path]:
        list_of_paths = list(self.path.glob('*chk*'))
        list_of_paths = [path.resolve() for path in list_of_paths]
        return sorted(list_of_paths)

    @property
    def number_of_checkpoints(self) -> int:
        return len(self.checkpoint_paths)

    def save_output_times(self, path: pathlib.Path | None = None) -> None:
        if path is None:
            path = self.path / 'output_times.txt'
        times = np.empty(self.number_of_checkpoints)
        for idx, checkpoint in enumerate(self.checkpoints):
            times[idx] = checkpoint.yt_dataset.current_time
